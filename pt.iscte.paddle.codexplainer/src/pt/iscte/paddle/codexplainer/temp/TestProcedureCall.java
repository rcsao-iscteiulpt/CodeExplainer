...
public class TestProcedureCall extends BaseTest {
	IProcedure test = module.addProcedure(INT);
	IVariableDeclaration v = test.addParameter(INT);
	IReturn ret = test.getBody().addReturn(INT.literal(0));
	...
	IProcedureCall recCall = sel.getAlternativeBlock().addCall(test, SUB.on(n, INT.literal(1)));
